...
const TaskListTable = ({ tableData }) => {
  ...
  const defaultStyle = "py-3 px-4 text-gray-800 font-medium text-[13px]";

  const defPadding = 'py-4 px-4'
  return (
    <div className="overflow-x-auto p-0 rounded-lg mt-3">
      <table className="min-w-full">
        <thead>
          <tr className="text-left">
            <th className={defaultStyle}>Name</th>
            <th className={defaultStyle}>Status</th>
            <th className={defaultStyle}>Priority</th>
            <th className={`${defaultStyle} hidden md:table-cell`}>
              Created On
            </th>
          </tr>
        </thead>
        <tbody>
          {tableData.map((task,index) => {
            return <tr key={index} className="border-t border-gray-200">
              ...
              <td className={`${defPadding} text-gray-700 text-[13px] text-nowrap hidden md:table-cell`}>
                {task.createdAt
